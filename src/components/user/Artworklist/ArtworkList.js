...
export default function Artworklist({ type }) {
    ...
    const handleClick = (artwork, active) => {
        if (slider) {
            ...
            active ? setSlider({ action: 'edit Artwork active', artwork: artwork }) : setSlider({ action: 'edit Artwork', artwork: artwork })
    ...
    const makeArtwork = (artwork, active) => {
        ...
        return <div className={style.itemRoot} key={artwork.id} onClick={() => { handleClick(artwork, active) }}>
    ...
    const getArtWorks = () => {
        ...
        if (artworks && options) {
            artworks.forEach((artwork) => {
                ...
                if (options.ArtworksActive.find((a) => { return a === artwork.id })) {
        ...
        return (
            <Section backgroundColor={'lila'}>
                <div className={style.root}>
                    ...
                    <p>Hier siehst du alle eingereichten Werke. Die Kuratoren bemühen sich um eine schnelle Freigabe</p>
                    <div className={style.noActive} >
                        {Eingereicht} <Button onClick={handleAddNewClick} label={'add New'}></Button>
                    </div>
                    {Active.length > 0 &&
                        <React.Fragment>
