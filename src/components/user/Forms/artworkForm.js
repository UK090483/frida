            ...
            >
                {({ isSubmitting, isValid, errors }) => (
                    <Form>
                        <Field name="picture">
                            {
                                ({ field /* { name, value, onChange, onBlur } */ }) => {
                                    if (field.value.src) {
                                        ...
                                        return < input {...field} type="text" placeholder="firstName" />
                                    }
                                }
                            }
                        </Field>
                        ...
                        <button className={`${formStyle.button} ${isValid ? '' : formStyle.diabled} `} type="submit" disabled={!isValid}>
                            Submit
                        </button>
                    </Form>
                )
...
const falidation = (values) => {
    ...
    if (!values.price) {
        ...
        values.price > 5000 || values.price < 100
    ) {
