...
export default function FridaImage({ artwork }) {
    ...
    const handleMouseMove = (e) => {
        ...
        let y = ((e.pageY - window.scrollY - imageClientRef.top) / imageClientRef.height) * -100
        setPos({ width: imageClientRef.width, height: imageClientRef.height, x: x, y: y, pageX: e.pageX, pageY: (e.pageY - window.scrollY) })
    }
        ...
        <div ref={RootRef} className={style.root}>
            ...
            <div className={`${style.magni} ${showGlass ? style.showGalss : ''}`} style={{ left: `${pos.pageX}px`, top: `${pos.pageY}px` }}>
                <img
                    ...
                    ref={loupImageRef}
                    srcSet={srcSet}
                    style={{ width: `${pos.width * SCALE[scale]}px`, height: `${pos.height * SCALE[scale]}px`, transform: ` translateX(${pos.x}%) translateY(${pos.y}%)` }}
                    src={src}
                    alt={`Kunstwerk ${artworkName} von ${artistName}`}
                >
                </img>
